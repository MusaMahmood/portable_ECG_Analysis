...
def get_model_seq2seq():
    model = Sequential()
    if ch1_only:
        model.add(k.layers.Reshape((seqlength, features, 1), input_shape=(input_shape,)))
    else:
        model.add(k.layers.Reshape((seqlength, features, 1), input_shape=input_shape))
...
def get_model():
    model = Sequential()
    if ch1_only:
        model.add(k.layers.Reshape((seqlength, features), input_shape=(input_shape,)))
...
with tf.device('/gpu:0'):  # switch to /cpu:0 to use cpu
    ...
    yy_predicted = tfs.maximize_output_probabilities(yy_predicted)  # Maximize probabilities of prediction.

    # Evaluate other dataset:
    yy_predicted2 = model.predict(xx_flex)
    yy_predicted2 = tfs.maximize_output_probabilities(yy_predicted2)  # Maximize probabilities of prediction.

    print('Elapsed Time (ms): ', tfs.current_time_ms() - start_time_ms)
    print('Elapsed Time (min): ', (tfs.current_time_ms() - start_time_ms) / 60000)

    if ch1_only:
        data_dict = {'x_val': xxv, 'y_val': yyv, 'y_out': yy_predicted, 'y_flex': yy_predicted2, 'x_flex': xx_flex}
    else:

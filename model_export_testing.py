...
LSTM_UNITS = 64
learn_rate = 0.005
description = 'tmp_rat_2c1d_d.lr' + str(learn_rate) + 'ep' + str(epochs) + '_v2'
...
def get_model():
    ...
    k_model.add(Conv1D(16, 8, strides=2, padding='same', activation='relu'))
    k_model.add(Conv1D(64, 8, strides=2, padding='same', activation='relu'))
...
with tf.device('/gpu:0'):
    ...
    if os.path.isfile(keras_model_name):
        ...
        if TRAIN:
            ...
            print("No model available, training disabled!")
            exit(-1)
